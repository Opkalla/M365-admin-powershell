...
# Replace the SkuId with the SkuId of the license you want to filter, and change the variable name if needed
# Replace the SkuId with the SkuId of the license you want to change to, and change the variable name if needed
...
Get-MgUser -Filter "assignedLicenses/any(x:x/skuId eq $($e3Sku.SkuId) )" | ForEach-Object {Set-MgUserLicense -user $_.UserPrincipalName -AddLicenses @{SkuId = $e5Sku.SkuId} -RemoveLicenses @($e3Sku.SkuId) }
